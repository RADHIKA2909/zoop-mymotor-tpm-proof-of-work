...
/* ---- Journey failure points ------------------------------------------- */

export type RiskLevel = 'lower' | 'moderate' | 'higher'

export const RISK_META: Record<RiskLevel, { label: string; tone: 'success' | 'warning' | 'error' }> = {
  lower: { label: 'Lower', tone: 'success' },
  moderate: { label: 'Moderate', tone: 'warning' },
  higher: { label: 'Higher', tone: 'error' },
}

export interface JourneyRiskStage {
  num: string
  icon: IconName
  title: string
  desc: string
...
export const JOURNEY_HEADER = {
  eyebrow: 'Journey failure points',
  title: 'Where reliability matters across the journey',
  lead: 'Based on the user journey and public feedback, these are the key moments where reliability issues can show up.',
  note: 'Illustrative reliability risk model based on journey + public feedback signals — not internal ZOOP data, and not measured failure rates.',
} as const

export const JOURNEY_STAGES: JourneyRiskStage[] = [
  {
    num: '01',
    icon: 'search',
    title: 'Discover',
    desc: 'Find chargers and check availability.',
    risk: 'moderate',
    potentialIssue: 'Stale availability information.',
  },
  {
    num: '02',
    icon: 'sliders',
